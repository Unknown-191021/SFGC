...
def main(args):
    ...
    if model_type != 'GAT':
        ...
        adj = adj.to_dense()
        data_full = GraphData(features, adj, labels)
        ...
        for it in range(0, args.num_experts):
            ...
            for e in range(args.teacher_epochs + 1):
                model.train()
                optimizer_model.zero_grad()
                _,output = model.forward(data_full)
                loss_buffer = F.nll_loss(output[data.idx_train], data_full.y[data.idx_train])
                ...
                if e % 20 == 0:
                    ...
                    _,output_val = model.forward(data_full)
                    loss_val = F.nll_loss(output_val[data.idx_val], labels_val)
                    acc_val = utils.accuracy(output_val[data.idx_val], labels_val)
            ...
            if len(trajectories) == args.traj_save_interval:
                n = 0
                while os.path.exists(os.path.join(args.log_dir, "replay_buffer_{}.pt".format(n))):
                    n += 1
                logging.info("Saving {}".format(os.path.join(args.log_dir, "replay_buffer_{}.pt".format(n))))
                torch.save(trajectories, os.path.join(args.log_dir, "replay_buffer_{}.pt".format(n)))
                trajectories = []
...
class Dpr2Pyg(InMemoryDataset):
    ...
    def process____(self):
        ...
        try:
            ...
            # edge_index = torch.LongTensor(dpr_data.adj.nonzero().cpu()).T
        except:
        ...
        try:
            ...
            # x = torch.FloatTensor(dpr_data.features.cpu()).float()
        except:
        ...
        try:
            ...
            # y = torch.LongTensor(dpr_data.labels.cpu())
        except:
            y = dpr_data.labels
    ...
    def process(self):
        ...
        if type(dpr_data.adj) == torch.Tensor:
            ...
            # adj_selfloop = dpr_data.adj + torch.eye(dpr_data.adj.shape[0])
            ...
            # edge_index = torch.LongTensor(adj_selfloop.nonzero())
            # edge_weight = torch.FloatTensor(adj_selfloop[adj_selfloop.nonzero()])
